...
class VideoWorker(QObject):
    ...
    playSound = pyqtSignal() # <<< ADDED: Signal to request sound playback
    ...
    def run(self):
        ...
        try:
            ...
            while self.running:
                ...
                # --- Handle logging and auto-saving based on the time cooldown ---
                if is_detecting_now:
                ...
                # --- Handle sound based on state change (the "rising edge" of detection) ---
                if is_detecting_now and not self.was_detecting_in_previous_frame:
                    if self.play_sound_on_detect:
                        self.playSound.emit() # <<< MODIFIED: Emit signal instead of playing sound
...
class MainWindow(QMainWindow):
    ...
    def start_video_thread(self, source_type: str, video_path: str | None):
        ...
        self.video_worker.playSound.connect(self.play_alert_sound) # <<< ADDED: Connect signal to slot
    ...
    # <<< ADDED: New method (slot) to play sound in the main thread
    def play_alert_sound(self):
        if winsound:
            try:
                # Use a more reliable system sound that is non-blocking
                winsound.PlaySound("SystemAsterisk", winsound.SND_ALIAS | winsound.SND_ASYNC)
            except Exception as e:
                print(f"[ERROR] Could not play winsound sound: {e}. Trying fallback.")
                print('\a', flush=True)
        else:
