...
class VideoWorker(QObject):
    ...
    # <<< MODIFIED: Reworked the entire function for robust ROI processing
    def process_frame(self, frame):
        ...
        if self.roi_enabled and self.roi_points:
            ...
            # Crop the image to the ROI for processing
            processing_gray = gray[y1:y2, x1:x2]
            
            # Draw the ROI rectangle on the result frame for visualization
            cv2.rectangle(result_frame, (x1, y1), (x2, y2), (255, 0, 255), 2)

        # Ensure the processing image is not empty
        ...
        for cnt in contours:
            ...
            if mnA < area < mxA:
                ...
                # Add the ROI offset back to the coordinates before drawing
        ...
        # Create a full-size mask for display
...
class MainWindow(QMainWindow):
    ...
    def eventFilter(self, source, event):
        if source is self.video_label and self.is_selecting_roi:
    ...
    def paintEvent(self, event):
        ...
        if self.is_selecting_roi and not self.roi_start_point.isNull():
            painter = QPainter(self)
            pen = QPen(Qt.GlobalColor.red, 2, Qt.PenStyle.DashLine)
            painter.setPen(pen)
            
            label_pos = self.video_label.pos()
            selection_rect = QRect(self.roi_start_point, self.roi_end_point).normalized()
            
            painter.drawRect(selection_rect.translated(label_pos))
