...
class VideoWorker(QObject):
    ...
    def __init__(self, source_type="camera", video_path=None, params=None,
        ...
        # State variables for auto-save and sound alerts
    ...
    def run(self):
        ...
        try:
            ...
            while self.running:
                ...
                # --- Handle logging, auto-saving, and sound based on cooldown ---
                        ...
                        if self.play_sound_on_detect:
                            if winsound:
                                try:
                                    ...
                                    print(f"[ERROR] winsound.Beep failed: {e}")
                            else:
                                print('\a', flush=True)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # NEW: prefs for checkboxes (persisted)
        self.pref_auto_save_on_detect = False
        self.pref_play_sound_on_detect = False
    ...
    def start_video_thread(self, source_type: str, video_path: str | None):
        ...
        # Initialize worker flags from current (persisted) checkboxes
        if hasattr(self, 'chk_auto_save'):
            self.video_worker.auto_save_on_detect = self.chk_auto_save.isChecked()
        else:
            self.video_worker.auto_save_on_detect = self.pref_auto_save_on_detect
        if hasattr(self, 'chk_play_sound'):
            self.video_worker.play_sound_on_detect = self.chk_play_sound.isChecked()
        else:
            self.video_worker.play_sound_on_detect = self.pref_play_sound_on_detect
    ...
    def create_controls_panel(self):
        ...
        # Test sound button (Beep 800Hz/100ms)
        self.btn_test_sound = QPushButton("ทดสอบเสียง (800Hz/100ms)")
        self.btn_test_sound.clicked.connect(self.test_sound)
        layout.addWidget(self.btn_test_sound)
    ...
    def on_auto_save_changed(self):
        ...
        # persist preference
        self.pref_auto_save_on_detect = self.chk_auto_save.isChecked()
        self.save_settings()
    ...
    def on_play_sound_changed(self):
        ...
        # persist preference
        self.pref_play_sound_on_detect = self.chk_play_sound.isChecked()
        self.save_settings()
    ...
    def update_param(self):
        ...
        self.save_settings()  # persist slider params continuously
    ...
    def test_sound(self):
        if winsound:
            try:
                winsound.Beep(800, 100)  # Frequency=800Hz, Duration=100ms
                print("[SOUND] Test beep 800Hz/100ms played.")
            except Exception as e:
    ...
    def load_settings(self):
        """Load settings.json into in-memory prefs; UI components may not exist yet."""
        try:
            if os.path.exists(SETTINGS_FILE):
                with open(SETTINGS_FILE, 'r') as f:
                    ...
                    # NEW: load checkbox prefs
                    self.pref_auto_save_on_detect = settings.get("auto_save_on_detect", self.pref_auto_save_on_detect)
                    self.pref_play_sound_on_detect = settings.get("play_sound_on_detect", self.pref_play_sound_on_detect)
                    print(f"[SETTINGS] Settings loaded from {SETTINGS_FILE}")
        except Exception as e:
            print(f"[ERROR] Could not load settings: {e}")

    def save_settings(self):
        """Persist current UI and prefs to settings.json."""
        try:
            auto_save = self.chk_auto_save.isChecked() if hasattr(self, 'chk_auto_save') else self.pref_auto_save_on_detect
            play_sound = self.chk_play_sound.isChecked() if hasattr(self, 'chk_play_sound') else self.pref_play_sound_on_detect
            settings = {
                "params": self.params,
                "cam_index": self.cam_index,
                "target_fps": self.target_fps,
                "cam_backend_name": self.cam_backend_name,
                # NEW: persist checkbox prefs
                "auto_save_on_detect": auto_save,
                "play_sound_on_detect": play_sound,
            }
    ...
    def update_controls_from_settings(self):
        ...
        # NEW: apply persisted checkbox prefs to UI
        if hasattr(self, 'chk_auto_save'):
            self.chk_auto_save.setChecked(self.pref_auto_save_on_detect)
        if hasattr(self, 'chk_play_sound'):
            self.chk_play_sound.setChecked(self.pref_play_sound_on_detect)
    ...
    def set_stylesheet(self):
        self.setStyleSheet("""
            ...
            QCheckBox { font-size: 14px; spacing: 5px; color: #E0E0E0; }
            QCheckBox::indicator { border: 1px solid #999; background-color: #555; width: 13px; height: 13px; border-radius: 2px; }
            QCheckBox::indicator:checked { background-color: #007ACC; border: 1px solid #005C99; }
