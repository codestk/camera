...
class VideoWorker(QObject):
    ...
    playSound = pyqtSignal() # Signal to request sound playback
    ...
    def __init__(self, source_type="camera", video_path=None, params=None,
        ...
        self.was_detecting_in_previous_frame = False
    ...
    def run(self):
        ...
        try:
            ...
            while self.running:
                ...
                if is_detecting_now and not self.was_detecting_in_previous_frame:
                    if self.play_sound_on_detect:
                        self.playSound.emit()
                
                self.was_detecting_in_previous_frame = is_detecting_now
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # <<< ADDED: Attributes to manage current source and session
        ...
        # Start with the default source (camera)
    ...
    def start_video_thread(self, source_type: str, video_path: str | None):
        ...
        self.video_worker = VideoWorker(
            ...
            session_save_path=self.session_save_path, # Use path from MainWindow
            session_timestamp=self.session_timestamp, # Use timestamp from MainWindow
        )
    ...
    # <<< MODIFIED: This function now creates a new session
    def switch_source(self, source_type: str, path: str | None):
        self.current_source_type = source_type
        self.current_source_path = path

        # Create a new session folder ONLY when switching sources
    ...
    def play_alert_sound(self):
        if winsound:
            try:
                ...
                print(f"[ERROR] Could not play winsound.Beep: {e}. Trying fallback.")
                print('\a', flush=True)
        else:
            print('\a', flush=True)

    # <<< MODIFIED: Now triggers re-processing for images
    def update_param(self):
        ...
        # If the current source is an image, re-process it with the new parameter
    ...
    def take_snapshot(self):
        ...
        with self.video_worker.lock:
            if self.video_worker.latest_result_frame is not None:
                ...
                session_path = self.video_worker.session_save_path
