...
class VideoWorker(QObject):
    ...
    def __init__(self, source_type="camera", video_path=None, params=None,
        ...
        # <<< MODIFIED: Separate timer for sound cooldown
        ...
        try:
            ...
            while self.running:
                ...
                if self.detection_enabled:
                    ...
                    # <<< MODIFIED: New sound logic with its own cooldown
                    if is_detecting_now and self.play_sound_on_detect:
                        current_time = time.time()
                        if current_time - self.last_sound_time > self.sound_cooldown_seconds:
                            self.playSound.emit()
                            self.last_sound_time = current_time # Reset sound timer
                    
                    if not is_detecting_now:
                        self.last_sound_time = 0 # Reset timer when no objects are detected
